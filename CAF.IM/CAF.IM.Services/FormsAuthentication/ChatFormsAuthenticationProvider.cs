...
{
    public class ChatFormsAuthenticationProvider : ICookieAuthenticationProvider
    {
        private readonly IRepository<ChatUserIdentity> _repositoryChatUserIdentity;
        private readonly IRepository<ChatUser> _repository;
        private readonly IMembershipService _membershipService;

        public ChatFormsAuthenticationProvider(IRepository<ChatUser> repository, IRepository<ChatUserIdentity> repositoryChatUserIdentity,
            IMembershipService membershipService)
        {
            _repository = repository;
            _repositoryChatUserIdentity = repositoryChatUserIdentity;
        ...
        private static void AddClaim(CookieResponseSignInContext context, ChatUser user)
        {
            ...
            // Add the Chat id claim
            context.Identity.AddClaim(new Claim(ChatClaimTypes.Identifier, user.Id));

            // Add the admin claim if the user is an Administrator
            if (user.IsAdmin)
            {
                context.Identity.AddClaim(new Claim(ChatClaimTypes.Admin, "true"));
            }
