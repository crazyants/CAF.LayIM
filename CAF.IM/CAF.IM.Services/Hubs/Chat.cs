...
{
    [AuthorizeClaim(ChatClaimTypes.Identifier)]
    public class Chat : Hub// INotificationService
    {
        ...
        public Chat(IRepository<ChatUser> repository,
                        ...
                        IRepository<Settings> settingsRepository,
                         ApplicationSettings settings,//ContentProviderProcessor resourceProcessor,
        ...
        private bool OutOfSync
        {
            get
            {
                ...
                return new Version(version) != Constants.ChatVersion;
            }
        }
        ...
        public void Join(bool reconnecting)
        {
            ...
            ChatUser user = _repository.GetById(userId);
        ...
        private void AddMentions(ChatMessage message)
        {
            ...
            {
                ChatUser mentionedUser = _repository.Table.FirstOrDefault(x => x.Name == userName);
                ...
                bool markAsRead = mentionedUser.Status != (int)UserStatus.Offline
                                  ...
                                  && _repository.IsUserInRoom(_chatRoomRepository, _cache, mentionedUser, message.Room);
        ...
        {
            var unread = _notificationRepository.GetUnreadNotificationsCount(mentionedUser);
        ...
        private void LogOn(ChatUser user, string clientId, bool reconnecting)
        {
            ...
            if (!reconnecting)
            {
                foreach (var r in user.AllowedRooms)
                {
                    ...
                    {
                        Name = r.Name,
                        Count = r.Users.Online().AsQueryable().Count(),
                        Private = r.Private,
                        Closed = r.Closed,
                        Topic = r.Topic
                    });
        ...
        {
            // New client state
            var ChatState = GetCookieValue("Chat.state");
