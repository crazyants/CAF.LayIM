...
namespace CAF.IM.Data
{
    ...
    public partial class MongoDBRepository<T> : IRepository<T> where T : BaseEntity
    {
        ...
        {
            return this._collection.Find(e => e.Key == id).FirstOrDefaultAsync().Result;
        }
        ...
        {
            var update = this._collection.ReplaceOneAsync(x=>x.Key == entity.Key, entity, new UpdateOptions() { IsUpsert = false }).Result;
            return entity;
        ...
        {
            this._collection.FindOneAndDeleteAsync(e => e.Key == entity.Key);           
        }
